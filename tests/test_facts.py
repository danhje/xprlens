...
def test_presolved_problem_is_detected_and_flagged(xp):
    ...
    # presolve() is itself deprecated in 9.8, and exercising it is the point of
    # this test. The CI deprecation guard only looks at src/xprlens, so a
    # deliberate call from a test does not trip it.
    p.presolve()
    after = read_shape(p)
